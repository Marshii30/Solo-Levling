...
export default function TaskList({ tasks, onAllDone, onTaskStart }) {
  const { state } = useGame();
  ...
  return (
    <>
      ...
      <div style={{ display: "grid", gap: 12 }}>
        {tasks.map((t) =>
          t.type === "timer" ? (
            <TimerCard key={t.id} task={t} onStart={onTaskStart} />
          ) : (
            <TaskItem key={t.id} task={t} onStart={onTaskStart} />
          )
        )}
      </div>
    </>
